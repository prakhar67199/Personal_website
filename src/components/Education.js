...
const Details = ({type,time,place,info,gpa,board}) => {
  ...
  return (
    ...
    >
      ...
      >
        <h3 
        className="capitalize font-bold text-2xl mb-2 text-primary
         dark:text-primaryDark sm:text-xl xs:text-lg">
          {type}
        </h3>
       <span className="capitalize font-medium text-xl text-dark/60 dark:text-light/60">
        {time} | {place} | {gpa} | {board}
       </span>
       <p className="font-bold text-sm text-dark/80 w-full dark:text-light ">
        {info}
       </p>
      </motion.div>
    </li>
    ...
    <div className="my-64 ">
      ...
      lg:w-[90%] md:w-full">
        <motion.div
          ...
          origin-top md:w-[2px] md:left-[30px] xs:left-[20px]"
        />
